...
for shows in tv_shows:
    if "scifi" in shows.lower():
        scifi_tv_shows.append(shows)
...
for i in range(len(new_tv_shows)):
    platform = new_tv_shows[i][5]
    if platform. lower() == "netflix":
        show_name = new_tv_shows[i][0]
        netflix_new_tv_shows.append(show_name)
...
for shows in cleaned_tv_shows:
    ...
    if shows[1].lower() == "crime":
        crime_show_data = [shows[0], shows[2], shows[4]]
        crime_genre.append(crime_show_data)
...
for show in cleaned_tv_shows:
    total_rotten_tomato_ratings += int(show[4])
print(f"\n4.2: total_ratings = {total_rotten_tomato_ratings}")

# TODO 4.3
num_of_shows = len(cleaned_tv_shows)
avg_rotten_tomato_ratings = round(total_rotten_tomato_ratings / num_of_shows, 1)
...
for show in cleaned_tv_shows_check:
   
    genre = show[1].strip().lower()
